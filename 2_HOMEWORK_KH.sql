...
-- 8. EMPLOYEE 테이블에서 실수령액이 5000만원 이상인 사원의 이름, 월급, 실수령액, 고용일 조회
...
SELECT EMP_NAME, SUBSTR(EMP_NO, 1, INSTR(EMP_NO, '-') -1) "주민번호 앞자리",
        SUBSTR(EMP_NO, INSTR(EMP_NO, '-') + 1, LENGTH(EMP_NO)) "주민번호 뒷자리",
        SUBSTR(EMP_NO, 1, INSTR(EMP_NO, '-') -1)  + SUBSTR(EMP_NO, INSTR(EMP_NO, '-') + 1, LENGTH(EMP_NO)) 합
FROM EMPLOYEE
WHERE EMP_ID = '201';

-- 24. EMPLOYEE테이블에서 부서코드가 D5인 직원의 보너스 포함 연봉 합 조회
SELECT SUM((SALARY + (SALARY * NVL(BONUS, 0))) * 12) "연봉 합"
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5';
