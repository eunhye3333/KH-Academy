...
FROM EMPLOYEE; -- 세미콜론 무조건 찍어야 함. KH 계정에서 진행하여야 함
...
-- EMPLOYEE 테이블의 직원 명, 이메일, 전화번호, 고용일 조회
...
SELECT EMP_NAME, SALARY*12 -- 컬럼명이 SALARY*12로 나옴(Result Set에 대한 컬럼명)
FROM EMPLOYEE;

-- EMPLOYEE 테이블에서 직원 명, 연봉, 보너스를 추가한 연봉 조회
SELECT EMP_NAME, SALARY*12, (SALARY*(1+BONUS))*12
FROM EMPLOYEE;
SELECT EMP_NAME, SALARY*12, (SALARY+(SALARY*BONUS))*12
FROM EMPLOYEE;
-- 보너스율이 NULL인 부분은 계산값이 NULL이라고 뜸
...
-- EMPLOYEE 테이블에서 이름, 고용일, 근무 일수(오늘 날짜(SYSDATE) - 고용일) 조회
SELECT EMP_NAME, HIRE_DATE, SYSDATE-HIRE_DATE -- SYSDATE에 시간까지 들어가 있어서 소수점이 나옴
FROM EMPLOYEE;

-- 컬럼에 별칭을 넣을 수 있음 (안 넣으면 어떤 데이터인지 구분하기 힘듦)
-- 컬럼명 AS 별칭        컬럼명 "별칭"        컬럼명 AS "별칭"        컬럼명 별칭
-- 별칭을 "" 써야 하는 경우 : 별칭에 특수문자(띄어쓰기 포함)가 들어간 경우 (꼭 "" 써야 함)
SELECT EMP_NAME AS 사원명, SALARY*12 연봉, (SALARY*(1+BONUS))*12 AS "보너스 포함 연봉"
FROM EMPLOYEE;

SELECT EMP_NAME "사원 명", SALARY*12 연봉, 
        (SALARY + (SALARY * BONUS))*12 AS "총수령액(보너스 포함)", 
        (SALARY + (SALARY * BONUS))*12 - (SALARY*12*0.03) AS 실수령액
FROM EMPLOYEE;

-- 리터럴 : 문자 그대로의 데이터
-- EMPLOYEE 테이블에서 직원 번호, 사원 명, 급여, 단위(원) 조회
-- 단위(원)은 테이블에 없음 -> 리터럴로 사용하면 테이블에서 가져온 것처럼 가져올 수 있음
SELECT EMP_ID, EMP_NAME, SALARY, '원' 단위
FROM EMPLOYEE;
-- Oracle에서는 ""은 별칭, ''는 문자(문자열)

-- DISTINCT : 중복을 제거함. 컬럼에 포함된 중복 값을 한 번씩만 표시하고자 할 때
-- 순서는 임의로 나옴
-- EMPLOYEE 테이블에서 직원의 직급 코드 조회
SELECT JOB_CODE FROM EMPLOYEE;
SELECT DISTINCT JOB_CODE FROM EMPLOYEE;

-- EMPLOYEE 테이블에서 부서 코드와 직급 코드 조회
SELECT DEPT_CODE, JOB_CODE
FROM EMPLOYEE;
--SELECT DISTINCT DEPT_CODE, DISTINCT JOB_CODE
--FROM EMPLOYEE;
-- DISTINCT는 SELECT절에 딱 한 번만 쓸 수 있음
SELECT DISTINCT DEPT_CODE, JOB_CODE -- 제일 앞에 쓰면 쌍으로 묶어서 중복 제거가 됨
FROM EMPLOYEE;

--SELECT EMP_NAME, DISTINCT DEPT_CODE, JOB_CODE -- 여러 개의 컬럼을 불러온 후 하나의 컬럼에서만 중복 제거 불가능
--FROM EMPLOYEE;
...
-- EMPLOYEE 테이블에서 부서 코드가 D9인 직원의 이름, 부서코드 조회
...
-- EMPLOYEE 테이블에서 퇴사 여부가 N인 직원을 조회화고 근무 여부를 재직 중으로 표시하여
-- 사번, 이름, 고용일, 근무 여부 조회
SELECT EMP_ID, EMP_NAME, HIRE_DATE, '재직 중' "근무 여부"
FROM EMPLOYEE
WHERE ENT_YN = 'N';
...
-- EMPLOYEE 테이블에서 실수령액(총수령액-(연봉*세금3%))이 5천만원 이상인 사원의 이름, 월급, 실수령액, 고용일 조회
SELECT EMP_NAME, SALARY, 
        (SALARY + (SALARY * BONUS))*12 - (SALARY*12*0.03) AS 실수령액,HIRE_DATE
FROM EMPLOYEE
...
-- EMPLOYEE 테이블에서 급여를 350만 이상 600만 이하를 받는 직원의 사번, 이름, 급여, 부서 코드, 직급코드 조회
SELECT EMP_ID, EMP_NAME, SALARY, DEPT_CODE, JOB_CODE
FROM EMPLOYEE
WHERE SALARY >= 3500000 AND SALARY <= 6000000;

-- EMPLOYEE 테이블에 월급이 4000000이상이고 JOB_CODE가 J2인 사원의 전체 내용 조회
SELECT *
FROM EMPLOYEE
...
-- AND와 OR 중에서는 AND가 우선 순위가 높음 
-- 따라서 위처럼 하면 DEPT_CODE가 D5이면서 고용일이 02년 1월 1일보다 빠른 사람이거나 DEPT_CODE가 D9인 사람이 됨
-- 그래서 소괄호를 꼭 붙여 줘야 한다
WHERE (DEPT_CODE = 'D9' OR DEPT_CODE = 'D5') AND HIRE_DATE < '02/1/1';

-- BETWEEN AND : 하한값 이상, 상한값 이하면 BETWEEN AND로 사용 가능
...
SELECT EMP_ID, EMP_NAME, SALARY, DEPT_CODE, JOB_CODE
FROM EMPLOYEE
--WHERE NOT SALARY BETWEEN 3500000 AND 6000000; -- 결과를 반전할 때 NOT을 붙여서 사용
WHERE SALARY NOT BETWEEN 3500000 AND 6000000; -- BETWEEN 앞에 NOT을 붙여도 됨
...
-- => '글%자' : 글로 시작해서 자로 끝나는 값 EX. 글자, 글의남자 등..
-- _ : 딱 1글자
-- => '_' : 한 글자
-- => '__' : 두 글자
-- => '___' : 세 글자
...
WHERE NOT EMP_NAME LIKE '김%'; -- 둘 다 상관없음

-- EMPLOYEE 테이블에서 이메일 중 _의 앞 글자가 3자리인 이메일 주소를 가진 사원의 사번, 이름, 이메일 주소 조회
SELECT EMP_ID, EMP_NAME, EMAIL
FROM EMPLOYEE
WHERE EMAIL LIKE '____%'; -- 원하는 결과가 출력되지 않음
-- 와일드카드 문자와 패턴의 특수 문자가 동일한 경우 어떤 것이 와일드카드고 어떤 것이 특수문자인지 구분할 수 없음
-- 특수 문자 자체를 데이터로 처리하기 위해 구분할 수 있는 장치 필요 => ESCAPE OPTION
SELECT EMP_ID, EMP_NAME, EMAIL
FROM EMPLOYEE
...
-- EMPLOYEE 테이블에서 메일 주소 _앞이 4글자이면서 DEPT_CODE가 D9 또는 D6이고, 
...
-- IS NULL/IS NOT NULL : 컬럼값에 NULL 값이 들어가 있는지 아닌지 판별해줌
...
WHERE NOT BONUS IS NULL; -- 둘 다 가능

-- EMPLOYEE 테이블에서 관리자도 없고 부서 배치도 받지 않은 직원의 이름, 관리자, 부서 코드 조회
...
WHERE JOB_CODE IN ('J1', 'J2', 'J3', 'J4'); -- OR 로 연결되는 값이 있다면 IN으로 묶을 수 있음
...
SELECT EMP_ID, EMP_NAME, SALARY || '원' "급여" -- 리터럴과도 연결 가능
FROM EMPLOYEE;

-- ORDER BY : 정렬할 때 작성하는 구문
-- SELECT문 가장 마지막에 작성, 실행 순서도 가장 마지막에 실행됨
-- 컬럼명/별칭/컬럼순번 정렬 방식 NULLS FIRST/NULLS LAST
-- 오름차순 : ASC(생략 가능) / 내림차순 : DESC(무조건 명시) -> 오름차순이 기본
...
--ORDER BY 이름 DESC; -- 제일 마지막에 실행되기 때문에 SELECT에 들어간 별칭을 인지 가능
--ORDER BY 3 DESC; -- 순번으로도 가능 SELECT에서의 순번을 이야기함
--ORDER BY BONUS DESC; -- NULL이 먼저 나옴
ORDER BY BONUS DESC NULLS LAST; -- NULL을 뒤로 보내고 정렬
